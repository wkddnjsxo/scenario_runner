...
class MORAI_gRPC:
    ...
    def __init__(self):
        ...
        try:
            if not rospy.core.is_initialized():
                rospy.init_node('grpc_vehicle_controller', anonymous=True)
        except:
            pass
        
        # reset_count 퍼블리셔
        self.reset_count_pub = rospy.Publisher('/reset_count', Int32, queue_size=1)
    ...
    def reset_vehicle(self):
        ...
        self.reset_count_pub.publish(msg)
        print(f"  · /reset_count 퍼블리시: {self.reset_count}")
        # 1) 위치 리셋
        self.set_vehicle_position(self.initial_position)

        # 2) 짧은 안정화 대기 -> 0.5초에서 2초로 늘림 (2024-06-17)
        time.sleep(2.0)
